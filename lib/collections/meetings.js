...
Meteor.methods({
  ...
  undoRequest(meetingId) {
    ...
    let userInfo = {userFBID: Meteor.user().services.facebook.id, userName: Meteor.user().profile.name};
    let update = {$pull: {requests: userInfo}};
    Meetings.update({_id: meetingId}, update);
  },
  ...
  finializeMeeting(meetingId, requestUserId) {
    ...
    let update = {$set: {bookedFBUser: requestUserId, bookedUserId: this.userId, booked: true}};
    Meetings.update({_id: meetingId}, update);
  },
  ...
  cancelMeeting(meetingId) {
    ...
    let update = {$set: {bookedFBUser: '', bookedUserId: '', booked: false}};
    Meetings.update({_id: meetingId}, update);
  },
